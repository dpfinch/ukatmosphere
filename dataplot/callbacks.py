...
from dataplot.DataTools import Ukraine_Map_Renderer
import pandas as pd
...
@app.callback(Output('fire_map_holder', 'children'),
        [Input('fire_map_toggle','value')],)
def fire_map_maker(toggle_value):
    if toggle_value:
        map_style = 'satellite-streets'
        map_name = 'Switch for map view'
    else:
    ...
    fig = Ukraine_Map_Renderer.render_map(app,map_style)
    graph = dcc.Graph(figure=fig, style={'width': '100%', 'height': '90vh'})
    return graph

@app.callback(Output('fire_map_toggle','label'),
        [Input('fire_map_toggle','value')],)
def fire_map_toggle(toggle_value):
    if toggle_value:
        map_name = 'Switch to map view'
    else:
        map_name = 'Switch to satellite view'
